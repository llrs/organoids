...
if (rm_outliers)  {
  saveRDS(comp_01, "data_out/samples_comparisons_wo_outliers.RDS")
} else {
...
if (rm_outliers)  {
  saveRDS(res, "data_out/limma_juanjo_wo_outliers.RDS")
} else {
...
if (rm_outliers)  {
  write.table(cm, file = "data_out/GETS_matrix_wo_outliers.tsv", quote = FALSE, sep = "\t", row.names = FALSE)
  system2("gzip", args = "-kf data_out/GETS_matrix_wo_outliers.tsv") # Compress it to upload to website
  write.table(out, file = "data_out/GETS_gene_wo_outliers.tsv", quote = FALSE, sep = "\t", row.names = FALSE, na = "")
  system2("gzip", args = "-kf data_out/GETS_gene_wo_outliers.tsv") # Compress it to upload to website
  write.table(m2,
              file = "data_out/GETS_sample_wo_outliers.tsv", quote = FALSE, sep = "\t", row.names = FALSE, na = " ")
  ...
  write.table(df, file = "data_out/GETS_colors_wo_outliers.tsv", quote = FALSE, sep = "\t", col.names = FALSE, row.names = FALSE)
} else {
